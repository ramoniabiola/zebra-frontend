import { HomeIcon, PlusIcon } from '@heroicons/react/24/outline';

const MyListingsPlaceholder = () => {
    return (
        <div className="h-full w-full flex flex-col items-center justify-center text-center py-16 px-4 mt-20">
            <div className="relative mb-8">
                <div className="w-18 h-18 bg-gradient-to-br from-sky-100 to-sky-200 rounded-full flex items-center justify-center shadow-lg">
                    <HomeIcon className="w-8 h-8 text-sky-600" />
                </div>
                <div className="absolute -top-1 -right-1 w-7 h-7 bg-gradient-to-br from-cyan-100 to-cyan-300 rounded-full flex items-center justify-center shadow-md">
                    <PlusIcon className="w-3.5 h-3.5 text-cyan-700" />
                </div>
            </div>

            <h2 className="text-xl font-bold text-gray-800 mb-3">
                No Listings Yet
            </h2>

            <p className="text-gray-600 text-sm mb-8 max-w-md leading-relaxed">
                Ready to share your space? Create your first apartment listing 
                and start connecting with potential tenants today!
            </p>
        </div>
    );
};

export default MyListingsPlaceholder;